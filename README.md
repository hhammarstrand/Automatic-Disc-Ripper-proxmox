...
- **Copy diagnostics**: one button produces everything needed to diagnose the install as a single paste, with keys and tokens removed. It carries the last few jobs *whatever their status* — with each title's duration, episode number and output name — because the interesting failures are the ones that finished successfully and did the wrong thing. Plus a whole-picture auto-eject section, and what the raw folder is holding.
